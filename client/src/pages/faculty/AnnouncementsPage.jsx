...
import { 
  ...
  ArrowRight, Search, Zap, MoreHorizontal, Trash2, Pencil
} from 'lucide-react';

const TARGET_ROLES = ['all', 'student', 'individual'];
...
const targetStyles = {
  ...
  individual: 'bg-violet-50 text-violet-700 border-violet-100',
};
...
const AnnouncementsPage = () => {
  usePageTitle('Faculty Announcements');
  ...
  const [form, setForm] = useState({
    ...
    target_role: 'student', target_user_id: '', expiry_date: '', 
  ...
  const fetchStudents = async () => {
    try {
      // Fetch students from the faculty's sections
      const sectionsRes = await api.get('/faculty/courses');
      const sections = sectionsRes.data;
      const studentMap = new Map();

      for (const section of sections) {
        const studentsRes = await api.get(`/faculty/sections/${section.section_id}/students`);
        if (studentsRes.data && Array.isArray(studentsRes.data)) {
          studentsRes.data.forEach(s => {
            const sid = s.user_id || s.student_id;
            if (sid && !studentMap.has(sid)) {
              studentMap.set(sid, { id: sid, name: s.name, email: s.email });
            }
          });
        }
      }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      if (editingId) {
        const res = await api.patch(`/faculty/announcements/${editingId}`, form);
        setList(list.map(a => a.announcement_id === editingId ? res.data : a));
        showToast('success', 'Announcement updated successfully!');
      } else {
        const res = await api.post('/faculty/announcements', form);
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/faculty/announcements/${confirmDelete}`);
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700 relative">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div>
          ...
          <p className="text-slate-500 font-bold text-[11px] uppercase tracking-widest leading-none ml-1">Broadcast section updates & critical alerts</p>
        </div>
        <button
          onClick={() => { resetForm(); setShowModal(true); }}
          className="flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-2xl text-[13px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all shadow-xl shadow-indigo-100 mt-4 lg:mt-0 active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 gap-6">
        {loading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-white border border-slate-100 rounded-[2rem] p-10 animate-pulse space-y-6">
          ...
          <div className="py-32 text-center bg-white border border-slate-200 border-dashed rounded-[3rem]">
            ...
            <h3 className="text-xl font-bold text-slate-900 tracking-tight mb-2">No Active Broadcasts</h3>
            <p className="text-slate-400 text-sm">Create your first announcement to reach your students.</p>
          </div>
        ) : (
          filteredList.map(a => (
            <div 
              ...
              className={`bg-white border border-slate-200 rounded-[2.5rem] p-7 lg:p-10 group hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 relative overflow-hidden cursor-pointer active:scale-[0.99] ${
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-8">
                <div className="flex flex-wrap items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest border border-slate-100 font-bold">
                    <Clock size={12} />
                    {new Date(a.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
                  </div>
                </div>
                {/* Actions (Only if created by current faculty) */}
                <div className="flex items-center gap-2 shrink-0">
                  {a.is_pinned && (
                    <div className="bg-amber-100 text-amber-600 p-2.5 rounded-xl shadow-sm border border-amber-200">
              ...
              <div className="max-w-4xl space-y-4">
                <h3 className="text-2xl lg:text-3xl font-black text-slate-900 tracking-tight group-hover:text-indigo-600 transition-colors leading-tight">
                  {a.title}
                </h3>
                <p className="text-base text-slate-500 leading-relaxed font-bold tracking-tight line-clamp-3">
                  {a.body}
                </p>
              </div>
              ...
              <div className="mt-10 pt-8 border-t border-slate-50 flex items-center justify-between">
                <div className="flex items-center gap-6">
                   <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Published By</span>
                        <span className="text-[12px] font-black text-slate-900">{a.author_name || 'Faculty Member'}</span>
                      </div>
                   </div>
                </div>
                <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.2em] text-indigo-600 group-hover:gap-4 transition-all">
                  Read Full Bulletin <ArrowRight size={16} />
                </div>
              </div>
            </div>
          ))
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 transition-all">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in-95 fade-in duration-300 flex flex-col">
            <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-indigo-50/50 sticky top-0 z-20">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-black text-slate-900 tracking-tight">{editingId ? 'Modify Bulletin' : 'Launch Broadcast'}</h2>
                  <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Section Communication Portal</p>
                </div>
              </div>
              <button 
              ...
              <form onSubmit={handleCreate} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full px-6 py-4 bg-slate-50 border border-slate-100 focus:border-indigo-500/30 focus:ring-4 focus:ring-indigo-500/5 rounded-2xl text-[13px] font-bold text-slate-900 transition-all placeholder:text-slate-300"
                  />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <select 
                      ...
                      className="w-full px-6 py-4 bg-slate-50 border border-slate-100 focus:border-indigo-500/30 focus:ring-4 focus:ring-indigo-500/5 rounded-2xl text-[13px] font-bold text-slate-900 transition-all appearance-none cursor-pointer"
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <select 
                      ...
                      onChange={e => setForm({ ...form, target_role: e.target.value, target_user_id: e.target.value === 'individual' ? form.target_user_id : '' })}
                      className="w-full px-6 py-4 bg-slate-50 border border-slate-100 focus:border-indigo-500/30 focus:ring-4 focus:ring-indigo-500/5 rounded-2xl text-[13px] font-bold text-slate-900 transition-all appearance-none cursor-pointer capitalize"
                    >
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    className="w-full px-6 py-4 bg-slate-50 border border-slate-100 focus:border-indigo-500/30 focus:ring-4 focus:ring-indigo-500/5 rounded-2xl text-[13px] font-bold text-slate-900 transition-all resize-none placeholder:text-slate-300 min-h-[160px]"
                  />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2 flex flex-col justify-end">
                    <div className="flex items-center gap-3 px-6 bg-amber-50 rounded-2xl border border-amber-100/50 h-[64px] transition-all hover:bg-amber-100/30">
                      <input 
                        type="checkbox" 
                        id="pin-toggle"
                        checked={form.is_pinned}
                        onChange={e => setForm({ ...form, is_pinned: e.target.checked })}
                        className="w-4 h-4 text-amber-600 rounded-lg border-amber-200 focus:ring-amber-500/20 cursor-pointer"
                      />
                      <label htmlFor="pin-toggle" className="flex-1 flex flex-col cursor-pointer ml-1">
                         <span className="text-[10px] font-black uppercase tracking-[0.1em] text-amber-700 leading-none mb-1">Priority Pin</span>
                         <span className="text-[9px] font-bold text-amber-600/70 leading-none">Stick to top</span>
                      </label>
                      <Pin size={14} className={`${form.is_pinned ? 'text-amber-500' : 'text-amber-200'} transition-colors`} />
                    </div>
                  </div>
                </div>

                <div className="space-y-2 pt-2">
                  <div className="flex items-center gap-3 px-6 bg-indigo-50 rounded-2xl border border-indigo-100/50 h-[64px] transition-all hover:bg-indigo-100/30">
                    <input 
                      type="checkbox" 
                      id="email-toggle"
                      checked={form.send_email}
                      onChange={e => setForm({ ...form, send_email: e.target.checked })}
                      className="w-4 h-4 text-indigo-600 rounded-lg border-indigo-200 focus:ring-indigo-500/20 cursor-pointer"
                    />
                    <label htmlFor="email-toggle" className="flex-1 flex flex-col cursor-pointer ml-1">
                       <span className="text-[10px] font-black uppercase tracking-[0.1em] text-indigo-700 leading-none mb-1">Email Notification</span>
                       <span className="text-[9px] font-bold text-indigo-600/70 leading-none">Notify via student inbox</span>
                    </label>
                    <Mail size={14} className={`${form.send_email ? 'text-indigo-500' : 'text-indigo-200'} transition-colors`} />
                  </div>
                </div>
                <div className="pt-4 flex gap-3">
                  ...
                  <button 
                    type="submit" 
                    disabled={submitting}
                    className="flex-[2] py-4 bg-slate-900 text-white rounded-2xl text-[13px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 disabled:opacity-50"
                  >
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 fade-in duration-300 flex flex-col relative">
            <div className="p-10 lg:p-14 text-slate-900 bg-slate-50 border-b border-slate-100 relative overflow-hidden">
              ...
              <button 
                ...
                className="absolute top-8 right-8 w-12 h-12 flex items-center justify-center rounded-2xl bg-white text-slate-400 hover:text-slate-900 shadow-sm border border-slate-200 transition-all z-20"
              >
                <X size={24} />
              </button>
              <div className="relative z-10 space-y-6 pr-12 lg:pr-0">
                <div className="flex items-center gap-3">
                ...
                <div className="flex flex-wrap items-center gap-6 pt-2">
                  <div className="flex items-center gap-3 text-slate-500">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[11px] font-black text-slate-900 capitalize">{selectedAnnouncement.target_role}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 text-slate-500">
                    <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center border border-slate-200 shadow-sm">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[11px] font-black text-slate-900">{new Date(selectedAnnouncement.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="p-10 lg:p-14 overflow-y-auto max-h-[50vh] bg-white scrollbar-hide">
              <p className="text-lg text-slate-600 leading-relaxed font-bold tracking-tight whitespace-pre-wrap">
            ...
            <div className="p-10 border-t border-slate-50 bg-slate-50/50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center text-white font-black text-sm">
                   {selectedAnnouncement.author_name?.[0] || 'F'}
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Bulletin Author</span>
                  <span className="text-[12px] font-black text-slate-900">{selectedAnnouncement.author_name}</span>
                </div>
              </div>
              <button 
                onClick={() => setSelectedAnnouncement(null)}
                className="px-8 py-4 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-200"
              >
                Close Bulletin
              </button>
            </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[200] flex items-center justify-center p-4 transition-all">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 fade-in duration-200 border border-slate-200">
            <div className="p-10 text-center space-y-6">
              <div className="w-20 h-20 bg-rose-50 rounded-[2rem] flex items-center justify-center mx-auto text-rose-500 shadow-lg shadow-rose-100 border border-rose-100">
                <Trash2 size={32} />
              </div>
              <div>
                <h3 className="text-2xl font-black text-slate-900 tracking-tight">Delete Bulletin?</h3>
                <p className="text-sm text-slate-500 font-bold mt-2 leading-relaxed px-4 tracking-tight">
                  This action will permanently retract this announcement for all recipients. This cannot be undone.
                </p>
              </div>
